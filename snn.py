...
from utils import Latency_coding
...
class SpikingPool:
    ...
    def __init__(self, input_shape, kernel_size, stride, padding=0):
        ...
        self.active_neurons = torch.ones(self.output_shape).to(torch.bool)
...
class SpikingConv:
    ...
    def lateral_inhibition(self, spks):
        ...
        # threshold 넘은 애들의 포텐셜 값 다 모아 놓기
    ...
    def __call__(self, spk_in, train=False):
        ...
        # output_spike 텐서에 true 인게 있냐 없냐
...
class CSNN:
    ...
    def __init__(self, input_shape):
        ...
        conv1 = SpikingConv(input_shape,
            ...
            nb_winners=1, firing_threshold=3, stdp_max_iter=None,
    ...
    def __call__(self, x, train_layer=None):
        ...
        sum_spks += spk_in.sum()
        spk = self.conv_layers[2](spk_in, train=(train_layer==1))
        sum_spks += spk.sum()
        if train_layer == 2 :
            return
        spk_out = self.pool_layers[2](spk)
        sum_spks += spk_out.sum()
        # output_spikes = spk_out
...
def main(
    ...
    epochs=[30, 30, 30], # Number of epochs per layer
    ...
    encoded_trainset = Latency_coding(num_train_samples, image_shape, trainLoader)
    encoded_testset = Latency_coding(num_test_samples, image_shape, testLoader)

    snn = CSNN(image_shape)
    ...
    output_train_max = np.zeros((len(encoded_trainset), np.prod(snn.output_shape)))
    # output_train_sum = np.zeros((len(encoded_trainset), np.prod(snn.output_shape)))
    ...
    output_test_max = np.zeros((len(encoded_testset), np.prod(snn.output_shape)))
    # output_test_sum = np.zeros((len(encoded_testset), np.prod(snn.output_shape)))
