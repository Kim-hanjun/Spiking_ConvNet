...
class SNN:
    ...
    def __init__(self, input_shape):
        ...
        conv2 = SpikingConv(pool1.output_shape,
            out_channels=100, kernel_size=3, stride=1, padding=2,
        ...
        conv3 = SpikingConv(pool2.output_shape,
            out_channels=200, kernel_size=3, stride=1, padding=2,
            nb_winners=1, firing_threshold=1, stdp_max_iter=None,
            adaptive_lr=True, inhibition_radius=1, v_reset=0,
        )
    ...
    def reset(self):
        # conv layer 의 reset 은 membrane potential, activity, recorded_spk(입력기록) 등을 초기화함 (새로운 data 가 들어올때마다)
        for layer in self.conv_layers:
            layer.reset()
        # pooling layer 의 reset 은 이 layer 에서의 neuron activity 를 초기화
        for layer in self.pool_layers:
            layer.reset()


    def __call__(self, x, train_layer=None):
        # sample 들 끼리 영향을 받지 않도록 필요한 것들 초기화
        self.reset()
...
def main(
    ...
    clf = LinearSVC(max_iter=3000, random_state=seed)
    clf.fit(output_train_mean,y_train)
    y_pred = clf.predict(output_test_mean)
    acc = accuracy_score(y_test,y_pred)
    print(f"Accuracy with method 3 (mean) : {acc}")
